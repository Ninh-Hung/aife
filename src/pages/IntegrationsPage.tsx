...
import { Bot, Inbox, Info, Mail, Plus, Power, RefreshCw, Send, Settings, ShieldCheck, Unplug } from 'lucide-react';
...
const isEmailSelectableAgent = (agent: Agent) => {
  ...
  return agent.isActive !== false && status !== 'archived' && status !== 'disabled' && ownerType !== 'INTERNAL';
};
...
export const IntegrationsPage: React.FC = () => {
  ...
  const loadEmailData = useCallback(async () => {
    ...
    try {
      const [accountsResponse, messagesResponse, draftsResponse, digestsResponse] = await Promise.all([
        listEmailAccounts(),
        listEmailMessages(selectedEmailAccountId ? { account_public_id: selectedEmailAccountId } : undefined),
        listEmailDrafts(),
        listEmailDigests(selectedEmailAccountId ? { account_public_id: selectedEmailAccountId } : undefined),
      ]);

      if (accountsResponse.success && accountsResponse.data) {
        setEmailAccounts(accountsResponse.data);
        const activeAccountId = selectedEmailAccountId || accountsResponse.data[0]?.public_id || '';
        setSelectedEmailAccountId(activeAccountId);
        if (!emailBindingAgentId) {
          const defaultBinding = accountsResponse.data
            .find((account) => account.public_id === activeAccountId)
            ?.bindings?.find((binding) => binding.is_default_handler);
  ...
  }, [emailAgentOptions, emailBindingAgentId, notification, selectedEmailAccountId]);
  ...
  const createRule = async () => {
    ...
    if (blacklistAction === 'auto_delete' && !window.confirm('Rule này sẽ tự chuyển email match vào Trash/Deleted Items. Tiếp tục?')) {
      return;
    }
    ...
    <Box className="h-full bg-slate-50 p-6 dark:bg-slate-900">
          ...
          <>
            <Box className="grid grid-cols-1 gap-3 lg:grid-cols-2">
              {emailAccounts.map((account) => (
                <Box
                  ...
                  className={`rounded border bg-white p-4 dark:bg-slate-800 ${
                    selectedEmailAccountId === account.public_id
                      ? 'border-emerald-500'
                ...
                >
                  <Box className="flex items-start justify-between gap-3">
                    <Box className="min-w-0">
                      ...
                      <Typography variant="body2" className="text-gray-600 dark:text-slate-400">
                        {account.provider} · {account.bindings?.find((binding) => binding.is_default_handler)?.agent_name || 'no default agent'}
                      </Typography>
                    </Box>
                    <Chip
                  ...
                  <Box className="mt-3 grid grid-cols-2 gap-3 text-sm text-gray-700 dark:text-slate-300">
                    <div>Last sync: {account.last_synced_at ? new Date(account.last_synced_at).toLocaleString() : '-'}</div>
                  ...
                  <Box className="mt-4 flex flex-wrap gap-2">
                    <Button size="small" onClick={() => setSelectedEmailAccountId(account.public_id)}>
                      Open
                    </Button>
                    <Button
                      size="small"
                    ...
                    <Button
                      ...
                      color="error"
                      startIcon={<Unplug size={15} />}
                      onClick={() => void disconnectSelectedEmail(account)}
                    >
                      Disconnect
                    </Button>
                  </Box>
                ...
                <Box className="space-y-3">
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      ...
                      onChange={(event) => setEmailBindingAgentId(event.target.value)}
                    >
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      ...
                      onChange={(event) => setEmailAccessLevel(event.target.value)}
                    >
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      ...
                      onChange={(event) => setEmailSummaryMode(event.target.value)}
                    >
                      <MenuItem value="off">off</MenuItem>
                      <MenuItem value="on_demand">on_demand</MenuItem>
                      <MenuItem value="proactive_digest">proactive_digest</MenuItem>
              ...
              <Box className="rounded border border-gray-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
                ...
                <Box className="space-y-3">
                  <TextField
                    ...
                    onChange={(event) =>
                      setRetentionForm((prev) => ({ ...prev, raw: Number(event.target.value) }))
                    }
                  ...
                  <TextField
                    ...
                    onChange={(event) =>
                      setRetentionForm((prev) => ({ ...prev, content: Number(event.target.value) }))
                    }
                  ...
                  <TextField
                    ...
                    onChange={(event) =>
                      setRetentionForm((prev) => ({ ...prev, vector: Number(event.target.value) }))
                    }
                    inputProps={{ min: 30, max: 365 }}
                  />
                  <Button
                ...
                <Box className="space-y-3">
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      ...
                      onChange={(event) => setBlacklistPatternType(event.target.value)}
                    >
                  ...
                  <TextField
                    ...
                    onChange={(event) => setBlacklistPatternValue(event.target.value)}
                  />
                  <FormControl fullWidth size="small">
                  ...
                  <Button fullWidth variant="outlined" onClick={() => void createRule()}>
                    Add rule
                  </Button>
                  <Box className="max-h-40 overflow-auto text-sm text-gray-700 dark:text-slate-300">
                    {emailRules.map((rule) => (
                      <Box key={rule.public_id} className="flex items-center justify-between border-t border-gray-100 py-2 dark:border-slate-700">
                        <span className="truncate">{rule.pattern_type}: {rule.pattern_value}</span>
                        <Chip size="small" label={rule.action} variant="outlined" />
                      </Box>
                    ))}
                  </Box>
                </Box>
                ...
                <Box className="max-h-80 space-y-3 overflow-auto">
                  {emailDrafts.length === 0 ? (
                    ...
                    emailDrafts.map((draft) => (
                      <Box key={draft.public_id} className="rounded border border-gray-100 p-3 dark:border-slate-700">
                        <Box className="flex items-start justify-between gap-2">
                          <Typography className="font-medium text-gray-900 dark:text-slate-100">
                            {draft.title || draft.message?.subject || 'Draft reply'}
                          </Typography>
                          <Chip size="small" label={draft.status} color={draft.status === 'failed' ? 'error' : 'default'} variant="outlined" />
                        </Box>
                        <Typography variant="body2" className="mt-1 line-clamp-3 whitespace-pre-line text-gray-600 dark:text-slate-300">
                          {draft.draft_text}
                        </Typography>
                        {draft.error_message && (
                        ...
                        <Box className="mt-3 flex gap-2">
                          {draft.status === 'failed' ? (
                            <Button size="small" variant="contained" onClick={() => void retryDraftSend(draft)}>
                              Retry send
                            </Button>
                            ...
                            <>
                              <Button size="small" onClick={() => openDraftEditor(draft)}>
                                Edit
                              </Button>
                              <Button size="small" variant="contained" onClick={() => void approveDraft(draft)}>
                                Approve/send
                              </Button>
                              <Button size="small" color="error" onClick={() => void rejectDraft(draft)}>
                                Reject
                              </Button>
                            </>
                          ) : null}
              ...
              {emailDigests.length === 0 ? (
                ...
                <Box className="grid grid-cols-1 gap-3 lg:grid-cols-2">
                  {emailDigests.slice(0, 6).map((digest) => (
                    <Box key={digest.public_id} className="rounded border border-gray-100 p-3 dark:border-slate-700">
                      <Box className="mb-2 flex items-start justify-between gap-2">
                        ...
                        <Chip size="small" label={`${digest.item_count} emails`} variant="outlined" />
                      </Box>
                      <Typography variant="body2" className="line-clamp-3 whitespace-pre-line text-gray-600 dark:text-slate-300">
                        {digest.summary || 'No summary'}
                      </Typography>
                      <Typography variant="caption" className="mt-2 block text-gray-500 dark:text-slate-400">
                        {digest.generated_at ? new Date(digest.generated_at).toLocaleString() : new Date(digest.created_at).toLocaleString()}
                      </Typography>
                    </Box>
                  ))}
                </Box>
            ...
            <Box className="rounded border border-gray-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
              <Box className="mb-3 flex items-center justify-between gap-3">
                ...
                <Button size="small" startIcon={<RefreshCw size={15} />} onClick={() => void syncSelectedEmail()}>
                  Sync now
                </Button>
              </Box>
              {latestSummary && (
              ...
              <Box className="divide-y divide-gray-100 dark:divide-slate-700">
                {emailMessages.length === 0 ? (
                  <Typography variant="body2" className="py-6 text-center text-gray-500 dark:text-slate-400">
                    No email message
                  </Typography>
                ) : (
                  emailMessages.map((message) => (
                    <Box key={message.public_id} className="grid grid-cols-1 gap-3 py-3 lg:grid-cols-[1fr_auto]">
                      <Box className="min-w-0">
                        <Box className="mb-1 flex flex-wrap items-center gap-2">
                          ...
                          <Chip size="small" label={message.status} variant="outlined" />
                        </Box>
                        <Typography variant="body2" className="text-gray-600 dark:text-slate-400">
                          {message.from_address} · {message.received_at ? new Date(message.received_at).toLocaleString() : '-'}
                        </Typography>
                        {message.snippet && (
                          <Typography variant="body2" className="mt-1 line-clamp-2 text-gray-500 dark:text-slate-400">
                            {message.snippet}
                          </Typography>
                        )}
                      </Box>
                      <Box className="flex flex-wrap items-center gap-2">
                        <Button size="small" onClick={() => void summarizeMessage(message)}>
                          Summarize
                        </Button>
                        <Button size="small" variant="outlined" onClick={() => void draftReply(message)}>
                          Draft reply
                        </Button>
                      </Box>
                    </Box>
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 3 }}>
          ...
          <Button onClick={() => void saveDraftEdits()} disabled={!editingDraft || emailActionLoading}>
            Save
          </Button>
          <Button
            color="error"
            onClick={() => void rejectEditedDraft()}
            disabled={!editingDraft || editingDraft.status !== 'pending_approval' || emailActionLoading}
          >
            Reject
          </Button>
          <Button
            ...
            disabled={!editingDraft || editingDraft.status !== 'pending_approval' || !draftText.trim() || emailActionLoading}
          >
            Approve/send
          </Button>
        </DialogActions>
